        ...
        //1.Start :添加附件
			...
			function uploadImage(obj){
				...
				//增加参数,点击咨询业务添加时,form表单的返回以及确认按钮不存在.因此增加一个标记参数
	             top.$.jBox.open("iframe:"+$('#ctx').val()+"/uploadImage/openWindow2", "上传照片（大小：100KB以内jpg；尺寸：295*413 单位像素以内）", 600, 500, {
	                ajaxData:{
	                	/*imagePid :imagePid,
	    				imageType:imageType,*/
	    				imageIds:imageIds,
        ...
        function uploads(obj){
            ...
            var tableType = $($(".attachmentImageTableType")).val();
            ...
            // 正常打开
            //增加参数,点击咨询业务添加时,form表单的返回以及确认按钮不存在.因此增加一个标记参数
            top.$.jBox.open("iframe:"+$('#ctx').val()+"/uploadImage/openWindow5", "打开）", 600, 500, {
                ajaxData:{
                    /*imagePid :imagePid,
                    imageType:imageType,*/
                    imageIds:imageIds,
        ...
        function deleteImageIds(imageId, obj){
            var imageAttachment   = $($(obj).parent().parent().parent().find(".imageAttachment")[0]); //得到隐藏的数据Id
            var imageParentIds = imageAttachment.val();  //input框中的数据
