...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/enterprise/auditAndReport/localWaitAuditList">地方待审核申请列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="enterpriseWorkers" action="${ctx}/enterprise/auditAndReport/localWaitAuditList" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="enterpriseWorkers" varStatus="index">
			<tr>
				...
				<td>
					<c:if test="${enterpriseWorkers.batchStatus eq '7'|| enterpriseWorkers.batchStatus eq '21'}">
						<input type="button" value="审核" onclick="audit('${enterpriseWorkers.id}','${enterpriseWorkers.personRecordId}')">
					</c:if>
					<%-- <c:if test="${enterpriseWorkers.batchStatus ne '7'}">
