...
<head>
	...
	<script type="text/javascript">
		...
		function page(n,s){
			$("#pageNo").val(n);
			$("#pageSize").val(s);
            $("#searchForm").attr("action","${ctx}/enterprise/auditAndReport/counselorList");
			$("#searchForm").submit();
        	return false;
        }
        ...
        function exportData(s){
            if (window.confirm("是否确定导出当前数据？")){
                // alert(s);
                if (s==10) {
                    $("#searchForm").attr("action","${ctx}/enterprise/auditAndReport/counselorList?export=1");
                    $("#searchForm").submit();
                }
            }
        }
...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/enterprise/auditAndReport/counselorList">有效咨询师列表</a></li>
		<li><a href="${ctx}/enterprise/auditAndReport/counselorOutList">准备转出咨询师列表</a></li>
		<li><a href="${ctx}/enterprise/auditAndReport/counselorInList">准备转入咨询师列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="enterpriseWorkers" action="${ctx}/enterprise/auditAndReport/counselorList" method="post" class="breadcrumb form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<ul class="ul-form">
			<li><label>姓名：</label>
				<form:input path="name" id="name" htmlEscape="false" maxlength="20" class="input-small"/>
			</li>
			<li><label>咨询师状态: </label>
				<form:select path="isValid" id="isValid" class="input-medium">
			...
			<li class="btns">
				...
				<input id="btnExport" class="btn btn-primary" type="button" value="导出" onclick="exportData(s=10)"/>
			</li>
