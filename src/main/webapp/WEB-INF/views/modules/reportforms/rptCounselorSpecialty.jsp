...
<head>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            $("#searchForm").validate({
                ...
                submitHandler: function(form){
                    ...
                    <%--window.location.href = "${ctx}/report/counselor/findReportCounselorSpecialtyInfo?Type=" + Type + "&batchNo=" + BatchNo;--%>
                    <%--form.action = "${ctx}/report/counselor/findReportCounselorSpecialtyInfo?Type=" + Type ;--%>
                    form.submit();
                },
        ...
        function page(n,s){
            ...
            if(BatchNo!=""){
                if (Type != 0){
                $("#searchForm").attr("action","${ctx}/report/counselor/findReportCounselorSpecialtyInfo?Type="+Type);
                loading('正在为您处理数据，请稍等...');
                $("#searchForm").submit();
            }}
            return false;
        ...
        function exportData(){
            ...
            if(BatchNo!=""){
                ...
                if (window.confirm("是否确定导出当前数据？")){
                    $("#searchForm").attr("action","${ctx}/report/counselor/exportReportCounselorSpecialtyInfoFile?Type="+Type);
                    $("#searchForm").submit();
                }}
            }return false;
        }
...
<ul class="nav nav-tabs">
    <li class="active"><a href="${ctx}/report/counselor/findReportCounselorSpecialtyInfo">变更专业人员</a></li>
</ul>
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
    ...
    <tr>
        ...
        <th style="text-align: center;" width="180">原登记主专业</th>
        <th style="text-align: center;" width="180">原登记辅专业</th>
        <c:if test="${fns:getDictValue(fResult, 'decaler_result', '')==1 }">
        <th style="text-align: center;" width="180">变更后登记主专业</th>
        <th style="text-align: center;" width="180">变更后登记辅专业</th>
        </c:if>
        <c:if test="${fns:getDictValue(fResult, 'decaler_result', '')==3 }">
        <th style="text-align: center;" width="180">现申请主专业</th>
        <th style="text-align: center;" width="180">现申请辅专业</th>
        </c:if>
        <th style="text-align: center;" width="280">登记执业单位</th>
    </tr>
    </thead>
    <tbody>
    ...
    <c:forEach items="${page.list}" var="reportCounselorCount" varStatus="status">
        <tr>
            ...
            <td>
                    ${reportCounselorCount.yuanzhu}
            </td>
            <td>
                    ${reportCounselorCount.yuanfu}
            </td>
            <td>
                    ${reportCounselorCount.xinzhu}
            </td>
            <td>
                    ${reportCounselorCount.xinfu}
            </td>
            <td>
                    ${reportCounselorCount.companyName}
            </td>
        </tr>
    </c:forEach>
