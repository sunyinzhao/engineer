...
<head>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $("#searchForm").validate({
                ...
                submitHandler: function(form){
                    ...
                    <%--window.location.href = "${ctx}/report/counselor/findReportCounselorCancelInfo?Type=" + Type + "&batchNo=" + BatchNo;--%>
                    <%--form.action = "${ctx}/report/counselor/findReportCounselorCancelInfo?Type=" + Type ;--%>
                    <%--form.action = "${ctx}/report/counselor/exportReportCounselorCancelInfoFile?Type=" + Type ;--%>
                    form.submit();
                },
        ...
        function page(n,s){
            ...
            if(BatchNo!=""){
                if (Type != 0){
            $("#searchForm").attr("action","${ctx}/report/counselor/findReportCounselorCancelInfo?Type="+Type);
            loading('正在为您处理数据，请稍等...');
            $("#searchForm").submit();
            }}
            return false;
        ...
        function exportData(){
            ...
            if (window.confirm("是否确定导出当前数据？")){
                $("#searchForm").attr("action","${ctx}/report/counselor/exportReportCounselorCancelInfoFile?Type="+Type);
                $("#searchForm").submit();
            }}
            }return false;
        }
...
<ul class="nav nav-tabs">
    <li class="active"><a href="${ctx}/report/counselor/findReportCounselorCancelInfo">注销登记人员</a></li>
</ul>
<form:form id="searchForm" modelAttribute="reportCounselorCount" method="post" class="breadcrumb form-search">
    ...
    <ul class="ul-form">
        ...
        <li class="btns">
            ...
            <%--javascript:{this.disabled=true;exportData();}--%>
        </li>
    </ul>
</form:form>
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
    ...
    <tr>
        ...
        <th style="text-align: center;" width="120">登记证书编号</th>
        <th style="text-align: center;" width="180">登记主专业</th>
        <th style="text-align: center;" width="180">登记辅专业</th>
        <th style="text-align: center;" width="280">原登记执业单位</th>
    </tr>
    </thead>
    <tbody>
    ...
    <c:forEach items="${page.list}" var="reportCounselorCount" varStatus="status">
        <tr>
            ...
            <td>
                    ${reportCounselorCount.zhuzhuanye}
            </td>
            <td>
                    ${reportCounselorCount.fuzhuanye}
            </td>
            <td>
